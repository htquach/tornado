...
class OptionParser(object):
    ...
    def generate_config_file(self, file=None):
        """Generate the content of the config_file using the options' default
        value
        """
        if file is None:
            file = sys.stderr
        print("### Content autogenerated on %s ###" % datetime.datetime.now(),
              file=file)
        by_group = {}
        for option in self._options.values():
            by_group.setdefault(option.group_name, []).append(option)
        ...
        for filename, o in sorted(by_group.items()):
            if filename:
                print("\n### Group: %s ###" % os.path.normpath(filename),
                      file=file)
            o.sort(key=lambda option: option.name)
            for option in o:
                oname = option.name
                ovalue = option.default
                if oname == "help":
                    continue
                if option.multiple and ovalue:
                    ovalue = "[%s]" % ovalue
                elif option.type is str:
                    ovalue = '"%s"' % option.default if option.default else '""'
                elif option.type in (datetime.datetime, datetime.timedelta):
                    #TODO: Implement output of datetime and timedelta
                    oname = "# %s" % oname
                elif ovalue is None or ovalue == "":
                    ovalue = "None"
                else:
                    ovalue = option.default
                print("%s = %s" % (oname, ovalue), file=file)
        print(file=file)
...
def generate_config_file(file=None):
    """Generate config file with the current value of all the command line
    options to stderr (or another file).

    See `OptionParser.generate_config_file`.
    """
    return options.generate_config_file(file)
